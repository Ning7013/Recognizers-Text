...
class ChineseDateParser(BaseDateParser):
    ...
    def parse(
        self, source: ExtractResult, reference: datetime = None
    ) -> Optional[DateTimeParseResult]:
    ...
    def parse_basic_regex_match(
        self, source: str, reference: datetime
    ) -> DateTimeParseResult:
        ...
        for regexp in self.config.date_regex:
            ...
            if (
                match
                and match.start() == 0
                and len(match.group()) == len(trimmed_source)
            ):
    ...
    def parse_implicit_date(
        ...
        if match and match.start() == 0 and len(match.group()) == len(trimmed_source):
            ...
            result.future_value = (
                result.past_value
            ) = DateUtils.safe_create_from_min_value(value.year, value.month, value.day)
            result.success = True
            return result
    ...
    def match_to_date(self, match, reference: datetime) -> DateTimeResolutionResult:
        ...
        if (
            month_str in self.config.month_of_year
            and day_str in self.config.day_of_month
        ):
        ...
        future_date, past_date = DateUtils.generate_dates(
            no_year, reference, year, month, day
        )
    ...
    def parser_duration_with_ago_and_later(
        ...
        if duration_res:
            ...
            if match:
                ...
                number_str = source[
                    duration_res.start : match.lastindex - duration_res.start + 1
                ]
                ...
                if src_unit in self.config.unit_map:
                    ...
                    if before_match and suffix.startswith(before_match[0]):
                        if unit_str == Constants.TIMEX_DAY:
                            ...
                            date = reference.replace(month=reference.month - 1 * number)
                        elif unit_str == Constants.TIMEX_YEAR:
                            date = reference.replace(year=reference.year - 1 * number)
                        else:
                            return result
